...
public static class Clay
{
    ...
    /// <remarks>
    ///     Ensure the byte span remains valid until referenced within render functions.
    ///     u8 string literals are allocated globally and will remain valid for the duration of the program.
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe void TextElement(ReadOnlySpan<byte> text, Clay_TextElementConfig c)
    {
        fixed (byte* pText = text)
        {
            TextElement(new Clay_String { chars = (sbyte*)pText, length = text.Length }, c);
        }
    }
    ...
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe Clay_ElementId Id(ReadOnlySpan<byte> text)
    {
        fixed (byte* pText = text)
        {
            return Id(new Clay_String { chars = (sbyte*)pText, length = text.Length });
        }
    }
    ...
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe Clay_ElementId Id(ReadOnlySpan<byte> text, int offset)
    {
        fixed (byte* pText = text)
        {
            return Id(new Clay_String { chars = (sbyte*)pText, length = text.Length }, offset);
        }
    }
    ...
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe Clay_ElementId IdLocal(ReadOnlySpan<byte> text)
    {
        fixed (byte* pText = text)
        {
            return IdLocal(new Clay_String { chars = (sbyte*)pText, length = text.Length });
        }
    }
    ...
    /// </remarks>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static unsafe Clay_ElementId IdLocal(ReadOnlySpan<byte> text, int offset)
    {
        fixed (byte* pText = text)
        {
            return IdLocal(new Clay_String { chars = (sbyte*)pText, length = text.Length }, offset);
        }
    }
